...
int main(int argc, char** argv) {
    ...
    if (test == "audio_fifo") {
        ...
        now = 3488; Mp3Run(); check(audioEvents.size() == 1, "opening duration plus margin has not elapsed");
        ...
        advance(3488); check(relay == HIGH && audioEvents.size() == 1, "opening completes before the next track");
        advance(1); check(relay == HIGH && audioEvents.back() == "play:1:3", "queued announcement begins at 3489ms without closing door early");
        advance(510); check(relay == HIGH, "door remains open until its close deadline");
        ...
        now = 4747; Mp3Run(); check(audioEvents == std::vector<String>{"play:4:1"},
        ...
        now += 2507; Mp3Run(); check(audioEvents.size() == 2, "V2 blockade intro retains its full duration and margin");
        ...
        now += 3488; Mp3Run(); check(audioEvents.size() == 1, "millis rollover cannot prematurely finish track");
