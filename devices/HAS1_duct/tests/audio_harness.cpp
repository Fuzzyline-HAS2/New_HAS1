...
int main(int argc, char** argv) {
    ...
    if (test == "audio_fifo") {
        ...
        check(Mp3TrackDurationMs(1, 2) == 3814, "measured opening track fixture");
        ...
        now = 4013; Mp3Run(); check(audioEvents.size() == 1, "opening duration plus margin has not elapsed");
        ...
        for (int i = 0; i < 4; ++i)
            check(audioStartTimes[i + 1] - audioStartTimes[i] >= Mp3TrackDurationMs(folders[i], files[i]) + 200,
        ...
        advance(3999); check(relay == HIGH && audioEvents.size() == 1, "opening still active before close deadline");
        advance(1); check(relay == LOW && current_time == 0 && audioEvents.size() == 1,
                          "door closes at 4000ms while opening audio still playing");
        advance(14); check(audioEvents.back() == "play:1:3", "queued announcement begins at4014ms");
        advance(1986); check(current_time == 2 && !duct_available, "cooldown timer progresses while queued speech plays");
        finished();
        ...
        now += 4013; Mp3Run(); check(audioEvents.size() == 1, "millis rollover cannot prematurely finish track");
