...
- **9 audio scenarios:** actual `audio_queue.ino`, phrase builders and measured `mp3_durations.h`, with only DFPlayer hardware replaced. Check V2 opening and blockade track completion boundaries, whole-phrase FIFO, queue limits, repetition, missing files/hardware, unsigned clock rollover, language selection, obsolete requests, and concurrent door/cooldown timers.

The duration table targets `HAS2-Nextion` branch `feat/audio-library-v2`, `audios/V2/duct_MP3_22k/01~08`; its source commit is recorded in the header. Durations are WAV frame counts divided by the sample rate, rounded up to milliseconds. The scheduler adds its `MP3_TRACK_MARGIN_MS` (100ms) margin separately.
