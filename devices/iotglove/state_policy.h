...
inline bool gameMutationsAllowed(const ServerSnapshot& state) {
  return state.phase == Phase::Active && state.deviceState != DeviceState::Setting &&
         state.deviceState != DeviceState::Ready && state.deviceState != DeviceState::Photo &&
...
// Preserve the photo/end safety overrides; setting/ready are explicit device
...
inline bool decodeServerStates(const char* game, const char* device, ServerSnapshot& out) {
  ...
  // The legacy exploration alias is retired. Reject it instead of treating it
  // as an active-game Other state, which could re-enable game mutations.
