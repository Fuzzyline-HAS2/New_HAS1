...
- `role=tagger` + `device_state=blink`: 술래 결정 전 보라 점멸. `activate`: 보라 상시 점등. 포획 허용은 기존 실제 술래 조회의 activate 조건을 유지한다.
- `device_state=setting/ready`: 기존 하양/빨강 색상에서 실제 칩 장착이면 4칸, 미장착이면 3칸. 준비 중 칩 변화로 포획·생명 증감 쓰기를 만들지 않는다.
