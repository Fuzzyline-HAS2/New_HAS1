...
| 준비 | 빨강, 칩 있음 4칸 / 없음 3칸 | device_state=ready |
| Photo 생존자·유령·neutral | 초록 4칸 | device_state=photo; 유령도 생존자로 표시 |
| Photo 술래 | 보라 4칸 | device_state=photo, role=tagger |
| 서버 응답 무효·Wi-Fi 단절·15초 만료 | 마지막 색 유지 | 쓰기 차단·진동 취소는 유지 |
...
- 서버 `device_state=photo`에서는 역할에 따른 색을 표시하지만 글러브 사용은 막는다. 삭제된 `exploration` 값은 무효 응답으로 처리한다.
