...
GitHub permits only a small number of unauthenticated API requests per hour.
If that shared limit is exhausted, set a `GH_TOKEN` (preferred) or
`GITHUB_TOKEN`; authentication is used only for `api.github.com` metadata and
is never forwarded to Release asset hosts.
...
The partition baseline is the table generated by the core build recipe from
`default.csv`. The packaged core 3.3.11 `default.bin` is a stale legacy layout
and is deliberately not used for this check.
