...
void loop() {
    ...
    // 스타터를 벗어난 채 열려 있는 기여도 세션을 정리한다. ptrCurrentMode() "앞"이어야 한다 —
    // 직전 프레임 끝의 TimerRun()->DataChanged()가 모드를 바꿨다면, 새 모드 함수가 한 번도
    // 돌기 전에 여기서 마무리된다. TimerRun() 뒤로 옮기면 폴링 GET 바로 뒤에 전송 GET이
    // 같은 프레임에 연달아 붙는다.
    ContribLoop();
